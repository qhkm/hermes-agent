...
class APIServerAdapter(BasePlatformAdapter):
    ...
    async def _handle_runs(self, request: "web.Request") -> "web.Response":
        ...
        # Persisted-transcript hydration for stable session ids: when the
        # client passes a session_id (e.g. the Telegram per-chat id from the
        # Jentera worker) with no explicit conversation_history or
        # previous_response_id, reload the stored transcript so follow-up
        # turns carry prior context — mirrors /api/sessions/{id}/chat.
        # Stateless runs (no client session id) skip the lookup; their fresh
        # run_id has no stored history to load, and a stale transcript must
        # never leak into a deliberately fresh run.
        if not conversation_history and (body.get("session_id") or stored_session_id):
